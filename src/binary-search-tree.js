...
class BinarySearchTree {
  ...
  has(value, treeHead = this.head) {
    ...
    return value > data ? this.has(value, right) : this.has(value, left);
  }
  ...
  max(treeHead = this.head) {
    ...
    if (treeHead.right) {
      return this.max(treeHead.right);
    }
    return treeHead.data;
  }
  ...
  remove(value, head = this.head) {
    ...
    if (!currentNodeWithParentNode) {
      return null;
    }
    let {
      cur: current,
      par: parent
    } = currentNodeWithParentNode;
    ...
    if (parent) {
      ...
      if (!left && !right) {
        return this.isHead(current) ? this.setNewTreeHead(null) : null;
      }
      if (!left) {
        return this.isHead(current) ? this.setNewTreeHead(right) : right;
      }
      if (!right) {
        return this.isHead(current) ? this.setNewTreeHead(left) : left;
      }
